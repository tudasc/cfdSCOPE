    ...
    for (size_t i = 0; i < U.getWidth(); i++) {
        for (size_t j = 0; j < U.getHeight(); i++) {
            for (size_t k = 0; k < U.getDepth(); i++) {
                ...
                builder.finishBlock(U.getGrid()->cellIndex(i, j, k), U.getGrid()->cellIndex(i, j, k));
            }
        }
    }
    ...
    for (size_t i = 0; i < U.getWidth(); i++) {
        for (size_t j = 0; j < U.getHeight(); j++) {
            for (size_t k = 0; k < U.getDepth(); k++) {
                auto f_x = - U.getLeftU(i, j, k) * U.dudx(i, j, k) - U.getTopV(i, j, k) * U.dudy(i, j, k) - U.getFrontW(i, j, k) * U.dudz(i,j,k);
                auto f_y = - U.getLeftU(i, j, k) * U.dvdx(i, j, k) - U.getTopV(i, j, k) * U.dvdy(i, j, k) - U.getFrontW(i, j, k) * U.dvdz(i,j,k);
                auto f_z = - U.getLeftU(i, j, k) * U.dwdx(i, j, k) - U.getTopV(i, j, k) * U.dwdy(i, j, k) - U.getFrontW(i, j, k) * U.dwdz(i,j,k);
                transportEq[idx++] = f_x;
                transportEq[idx++] = f_y;
                transportEq[idx++] = f_z;
            }
...
template<typename T>
inline Vec3<T> traceBackward(Vec3<T> endPos, const VelocityField<T>& U, float dt) {
    // Use 2nd order explicit Runge-Kutte scheme to trace velocity "particle" back in time
...
template<typename T>
inline VelocityField<T> semiLagrangianAdvection(const VelocityField<T>& U, double dt) {
    auto U_adv = U;
...
template<typename T>
inline PressureField<T> solvePressureCorrection(const VelocityField<T>& U_adv, const PressureField<T>& p, double dt) {
...
template<typename T>
inline VelocityField<T> applyPressureCorrection(const VelocityField<T>& U_adv, const PressureField<T>& p, double dt) {
    ...
    for (size_t i = 0; i < width; i++) {
        for (size_t j = 0; j < height; j++) {
            for (size_t k = 0; k < depth; k++) {
                auto u = U_adv.getLeftU(i, j, k) - dt * (p.getPressure(i, j, k) - p.getPressure(i-1, j, k)) / cellSize;
                auto v = U_adv.getTopV(i, j, k) - dt * (p.getPressure(i, j, k) - p.getPressure(i, j-1, k)) / cellSize;
                auto w = U_adv.getFrontW(i, j, k) - dt * (p.getPressure(i, j, k) - p.getPressure(i, j, k-1)) / cellSize;               
...
int main(int argc, char** argv) {
    ...
    for (double t = 0; t < endTime; t+=dt) {
        ...
        // - Write field 
        write_to_file(U_corr, p_new, "fields_" + std::to_string(step));
    }
